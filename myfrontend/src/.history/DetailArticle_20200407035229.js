...
class DetailArticle extends Component {
    ...
    async componentDidMount() {
        ...
        var url = '';
        if (query.web_url) {
            url = "https://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/article?id=" + query.id + '&web_url=' + query.web_url;
            this.setState({Guardian: false})
            this.setState({check: false})
        } else {
            url = "https://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/article?id=" + query.id;
    ...
    showContent() {
        if (this.state.showMore) {
            var len = this.state.data.short_desc.short_desc.length
            if (this.state.data.short_desc.short_desc.substring(len-2, len - 3) === '.') {
                if(this.state.data.short_desc.short_desc.substring(len-1,len-2) === '.') {
                    return this.state.data.short_desc.substring(0, len - 2)
                } else {
                    return this.state.data.short_desc.substring(0, len - 1)
                }
            }     
    ...
    showContentNY() {
        if (this.state.showMore) {
            var len = this.state.data.docs[0].short_desc.length
            if (this.state.data.docs[0].short_desc.substring(len-2, len - 3) === '.') {
                if(this.state.data.docs[0].short_desc.substring(len-1,len-2) === '.') {
                    return this.state.data.short_desc.substring(0, len - 2)
                } else {
                    return this.state.data.short_desc.substring(0, len - 1)
                }
                                    ...
                                    <div className="card">
                                        <div className="card-veritical">
                                            <div className="titles">
                                                <h4 className="card-title" style={{fontWeight: '500', margin: 0, color: {r:5,g:87,b:87}, fontStyle: 'italic', fontSize: 20}}>{this.state.Guardian ? this.state.data.webTitle : this.state.data.docs[0].headline.main}</h4>
