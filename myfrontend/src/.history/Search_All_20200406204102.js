...
export default class Search_All extends Component {
    ...
    async componentDidMount() {
        const guardian_Url = 'http://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/Search?id=corona&source=guardian';
        const response = await fetch(guardian_Url);
        const data = await response.json();
        this.setState({data: JSON.stringify(data.temp)});

        const ny_Url = 'http://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/Search?id=corona&source=ny';
