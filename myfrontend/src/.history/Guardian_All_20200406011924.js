...
export default class Guardian_Home extends Component {
    ...
    async componentDidMount() {
        ...
        const url3 = "/Guardian_Business";
        const response3 = await fetch(url3);
        const data3 = await response3.json();
        this.setState({data3: JSON.stringify(data3.temp)});

        const url4 = "/Guardian_Politics";
        const response4 = await fetch(url4);
        const data4 = await response4.json();
        this.setState({data4: JSON.stringify(data4.temp)});

        const url5 = "/Guardian_Technology";
        const response5 = await fetch(url5);
        const data5 = await response5.json();
        this.setState({data3: JSON.stringify(data5.temp)});

        const url6 = "/Guardian_Sports";
