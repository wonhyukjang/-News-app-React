...
export default class NY_All extends Component {
    ...
    async componentDidMount() {
        ...
        const url2 = "https://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/NY_World";
        const response2 = await fetch(url2);
        const data2 = await response2.json();
        this.setState({data2: JSON.stringify(data2.temp)});

        const url3 = "https://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/NY_Business";
        const response3 = await fetch(url3);
        const data3 = await response3.json();
        this.setState({data3: JSON.stringify(data3.temp)});

        const url4 = "https://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/NY_Politics";
        const response4 = await fetch(url4);
        const data4 = await response4.json();
        this.setState({data4: JSON.stringify(data4.temp)});

        const url5 = "https://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/NY_Technology";
        const response5 = await fetch(url5);
        const data5 = await response5.json();
        this.setState({data5: JSON.stringify(data5.temp)});

        const url6 = "https://Nodeserver-env-1.eba-4mmhkpip.us-west-2.elasticbeanstalk.com/NY_Sports";
